...
class Decoder(nn.Module):
    def __init__(self, output_size, emb_size, hidden_size, num_layers, dropout):
        ...
        self.rnn = nn.LSTM(
            hidden_size * 2 + emb_size, hidden_size,
    ...
    def forward(self, x, encoder_states, hidden, cell):
        ...
        hid = hidden
        hid = hid[-1].unsqueeze(0)
        
        
        

        # hidden: (num_layers, batch, H)
        h_repeated = hid.repeat(seq_len, 1, 1)    # (seq, batch, H)
        ...
        energy = torch.cat((h_repeated, encoder_states), dim=2)
        energy = self.relu(self.energy(energy))      # (seq, batch, 1)

        # Attention weights
        attention = self.softmax(energy)             # softmax over seq
        attention = attention.permute(1, 2, 0)       # (batch, 1, seq)

        enc = encoder_states.permute(1, 0, 2)        # (batch, seq, 2H)

        # context = weighted sum of encoder states
        context = torch.bmm(attention, enc)          # (batch, 1, 2H)
        context = context.permute(1, 0, 2)           # (1, batch, 2H)

        # rnn input: concat(context, embedded)
        rnn_input = torch.cat((context, embedded), dim=2)  # (1,batch,2H+E)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg):
        ...
        for t in range(1, trg_len):
            pred, hidden, cell = self.decoder(x, encoder_states, hidden, cell)
            outputs[t] = pred
...
def predict(model,sentence):
    ...
    for _ in range(max_len):
        with torch.no_grad():
            out, hidden, cell = model.decoder(x , output, hidden, cell)
